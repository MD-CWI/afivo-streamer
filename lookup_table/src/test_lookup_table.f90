program test
  ...
  write (6,'(1x,A)') "You should see 2nd order convergence"
  write (6,'(1x,A,i20)') "Number of lookups performed :", cntr * test_size
  write (6,'(1x,A,f20.2)') "Number of lookups / second  :", &
           (cntr * test_size) / (total_time + epsilon(1.0_dp))
  write (6,'(1x,A,f20.3)') "Nanosecond per lookup       :", &
           1e9_dp * total_time / (cntr * test_size)
  write (6,'(A)') ""

  write (6,'(A)') ""
  write (6,'(1x,A)') "Testing multiple column mode (2 columns)"
  write (6,'(1x,A)') " *** Vectorized calls (one call for all the data) ***"
  write (6,'(1x,A)') "Iteration    table_size   max. difference"
