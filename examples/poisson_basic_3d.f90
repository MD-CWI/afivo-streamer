...
program poisson_basic_3d
  ...
  ! The manufactured solution exists of two Gaussians, which are stored in gaussian
  if (n_gaussian == 1) then
     ! Amplitudes:  [1.0_dp]
     ! Sigmas    :  [0.04_dp]
     ! Locations :  [0.5_dp, 0.5_dp, 0.5_dp]
     call gauss_init(gaussian, [1.0_dp], [4.00E-2_dp], &
          reshape([0.5_dp, 0.5_dp, 0.5_dp], [3,1]))
     write(*,"(2(A17,2x,i2,/),2(A17,1x,Es10.2,/),A17,1x,3(Es10.2,1x))") &
     ...
     write(*,"(2(A17,2x,i2,/),2(A17,2(1x,Es10.2),/),A17,1x,6(Es10.2,1x))") &
       "gaussian%n_gauss:",gaussian%n_gauss, &
       "gaussian%n_dim  :",gaussian%n_dim, &
       "gaussian%ampl   :",gaussian%ampl(:), &
       "gaussian%sigma  :",gaussian%sigma(:), &
  ...
  write(fname, "(A,I0)") "poisson_basic_3d_", 0
  ...
  do mg_iter = 1, n_iterations
     ...
     write(*,"(I8,13x,2Es14.5)") mg_iter, maxval(abs(residu)), &
          maxval(abs(anal_err))
    ...
    outer: do k = 1, nc
       do j = 1, nc
          do i = 1, nc
             ...
             drhs = dr2 * gauss_4th(gaussian, xyz) / 12
    ...
    do k = 1, nc
       do j = 1, nc
          do i = 1, nc
             ...
             box%cc(i, j, k, i_rhs) = gauss_laplacian(gaussian, xyz)
          end do
       end do
    end do
    ...
    case (1)
       do k = 1, nc
          do j = 1, nc
             ...
             box%cc(ix, j, k, iv) = gauss_value(gaussian, xyz)
          end do
       end do
    case (2)
       do k = 1, nc
          do i = 1, nc
             ...
             box%cc(i, ix, k, iv) = gauss_value(gaussian, xyz)
          end do
       end do
    case (3)
       do j = 1, nc
          do i = 1, nc
             ...
             box%cc(i, j, ix, iv) = gauss_value(gaussian, xyz)
          end do
       end do
    end select
