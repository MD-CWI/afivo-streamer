  ...
  do
     ...
     if (write_out) then
        call a3_write_silo(tree, fname, ST_out_cnt, ST_time, &
             ixs_cc=[i_electron, i_pos_ion, i_electric_fld, i_phi, i_photo], dir=ST_output_dir)
        call a3_write_plane(tree, fname, [i_electron, i_phi, i_electric_fld], &
...
contains
  ...
  subroutine refine_routine(boxes, id, refine_flag)
    ...
    max_electric_fld = maxval(boxes(id)%cc(1:nc, 1:nc, 1:nc, i_electric_fld))
    alpha            = LT_get_col(ST_td_tbl, i_alpha, max_electric_fld)
  ...
  real(dp) function get_max_dt(tree)
    ...
    real(dp)               :: max_electric_fld, min_electric_fld, max_dns, dr_min
    real(dp)               :: mobility, diff_coeff, alpha, max_mobility
    real(dp)               :: dt_cfl, dt_dif, dt_drt, dt_alpha

    call a3_tree_max_cc(tree, i_electric_fld, max_electric_fld)
    call a3_tree_min_cc(tree, i_electric_fld, min_electric_fld)
    call a3_tree_max_cc(tree, i_electron, max_dns)

    dr_min       = a3_min_dr(tree)
    mobility     = LT_get_col(ST_td_tbl, i_mobility, max_electric_fld)
    max_mobility = LT_get_col(ST_td_tbl, i_mobility, min_electric_fld)
    diff_coeff   = LT_get_col(ST_td_tbl, i_diffusion, max_electric_fld)
    alpha        = LT_get_col(ST_td_tbl, i_alpha, max_electric_fld)

    ! CFL condition
    dt_cfl = sqrt(1/3.0_dp) * dr_min / (mobility * max_electric_fld)
  ...
  subroutine electric_field_from_potential(box)
    ...
    box%cc(1:nc, 1:nc, 1:nc, i_electric_fld) = 0.5_dp * sqrt(&
         (box%fx(1:nc  , 1:nc  , 1:nc  , electric_fld) + &
          box%fx(2:nc+1, 1:nc  , 1:nc  , electric_fld))**2 + &
         (box%fy(1:nc  , 1:nc  , 1:nc  , electric_fld) + &
          box%fy(1:nc  , 2:nc+1, 1:nc  , electric_fld))**2 + &
         (box%fz(1:nc  , 1:nc  , 1:nc  , electric_fld) + &
          box%fz(1:nc  , 1:nc  , 2:nc+1, electric_fld))**2)
  ...
  subroutine update_solution(box, dt)
    ...
    real(dp)                    :: inv_dr, src, electric_fld
    ...
    do k = 1, nc
       do j = 1, nc
          do i = 1, nc
             electric_fld = box%cc(i, j, k, i_electric_fld)
             loc          = LT_get_loc(ST_td_tbl, electric_fld)
  ...
  !> Set the photoionization for tree
  ...
  subroutine set_photoionization_rate(box, coeff)
    ...
    integer                     :: i, j, k, nc
    real(dp)                    :: electric_fld, alpha, mobility, tmp
    type(LT_loc_t)              :: loc
    ...
    do k = 1, nc
       do j = 1, nc
          do i = 1, nc
             ...
             tmp = electric_fld * mobility * alpha * box%cc(i, j, k, i_electron) * coeff(1)
             if (tmp < 0) tmp = 0
