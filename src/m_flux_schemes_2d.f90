...
contains
  ...
  !> Compute diffusive flux (second order)
  subroutine flux_diff_2d(box, dc, iv)
    type(box2_t), intent(in)  :: box
    integer, intent(in)        :: iv
    real(dp), intent(inout)    :: dc(1:box%n_cell+1, 1:box%n_cell+1, 2)
    integer                    :: nc, d, i, j
    ...
    do j = 1,  nc+1; do i = 1,  nc+1
       do d = 1, 2  
          dc(i, j, d) = - dc(i, j, d) * gradient_2d(box, 1, [i, j], 2*d-1, iv)
       end do
    end do; end do
  ...
  function gradient_2d(box, cc_num, ix, dir, iv) result(grad)
    type(box2_t), intent(in)         :: box
    integer, intent(in)               :: cc_num !< 2 if cc is assumed continuous 1 otherwise
    integer, intent(in)               :: ix(2), dir, iv
    real(dp)                          :: grad, inv_dr, s_C, harm_ep, med, fac
    integer                           :: n_ix(2)
    
    n_ix      = a2_neighb_dix(:, dir)
    inv_dr    = 1/box%dr
    med       = a2_harm_w(1.0_dp, ST_epsilon_die, 0.5_dp)
 

    
    if (a2_border(box%cc(ix(1), ix(2), i_eps), box%cc(ix(1)+n_ix(1), ix(2)+n_ix(2), i_eps), med)) then
      if (cc_num == 2) then
      ...
      grad     = inv_dr*a2_neighb_high_pm(dir)*(box%cc(ix(1)+n_ix(1), ix(2)+n_ix(2), iv) - &
                 box%cc(ix(1), ix(2), iv))
    end if

  end function gradient_2d

end module m_flux_schemes_2d
