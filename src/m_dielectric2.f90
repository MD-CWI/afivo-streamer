...
!   integer, allocatable, public, protected :: box_id_to_surface_id(:)
  ...
  public :: dielectric2_initialize
  public :: dielectric2_update_surface_charge
  public :: dielectric2_photon_emission
  public :: dielectric2_combine_substeps
  public :: dielectric2_photon_absorption
  public :: dielectric2_reset_photons
  ! public :: dielectric_surcharge_output
  ! public :: dielectric_set_preset_charge

contains

  subroutine dielectric2_initialize(tree, cfg)
    use m_config
...
!          "Preset surface charge")
  ...
  end subroutine dielectric2_initialize

!   subroutine dielectric2_allocate(tree)
!     use m_config
!     type(af_t), intent(in) :: tree
...
!   end subroutine dielectric2_allocate
...
!   subroutine find_dielectric2_neighbor(tree, id, id_diel, nb, nb_eps)
!     use m_streamer
!     type(af_t), intent(in) :: tree
...
!   end subroutine find_dielectric2_neighbor
...
!   subroutine dielectric2_update_after_refinement(tree, ref_info)
!     use m_advance_base
!     type(af_t), intent(in)       :: tree
...
!           call find_dielectric2_neighbor(tree, id, id_diel, nb, eps)
...
!              !print *, "before prolong", surface_list(ix)%charge
!              call prolong_surface_from_parent(tree, surface_list(ix))
!              !print *, "after prolong", surface_list(ix)%charge
!           end if
!        end do
!     end do

!   end subroutine dielectric2_update_after_refinement
  
!   subroutine dielectric2_set_preset_charge(tree)
!     use m_advance_base
!     type(af_t), intent(in)       :: tree
!     integer                      :: ix
    
!     do ix = 1, size(surface_list)
!         if (surface_list(ix)%in_use) then
!             call set_preset_charge_to_box(tree, surface_list(ix))
!         end if
!     end do

!   end subroutine dielectric2_set_preset_charge
...
!   subroutine dielectric2_fix_refine(tree, ref_flags)
!     use m_streamer
!     type(af_t), intent(in) :: tree
...
!        end do
!     end do
!   end subroutine dielectric2_fix_refine

!   subroutine dielectric2_rearrange_charge(tree, s_in)
...
!     end do
!   end subroutine dielectric2_rearrange_charge
...
!   subroutine dielectric2_adjust_field(boxes, id, s_in)
!     use m_units_constants
!     use m_streamer
!     type(box_t), intent(inout) :: boxes(:)
!     integer, intent(in)        :: id
...
!   end subroutine dielectric2_adjust_field

  subroutine dielectric2_update_surface_charge(box, surface, dt, s_in, s_out)
    use m_units_constants
    use m_streamer
  ...
  end subroutine dielectric2_update_surface_charge

  subroutine dielectric2_photon_emission(box, surface, dt, s_in, s_out)
    use m_units_constants
    use m_streamer
    type(box_t), intent(inout)          :: box
    type(surface_t), intent(inout) :: surface
  ...
  subroutine dielectric2_photon_absorption(tree, xyz_start, xyz_end, n_dim, &
  ...
  end subroutine dielectric2_photon_absorption
  ...
  subroutine dielectric2_reset_photons()
    integer :: n
...
!   subroutine dielectric2_surcharge_output(tree, output_name, output_cnt, wc_time)
!     use m_mrgrnk
!     use m_init_cond
...
!   end subroutine dielectric2_surcharge_output

end module m_dielectric2
