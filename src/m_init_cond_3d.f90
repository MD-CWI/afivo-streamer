module m_init_cond_3d
  ...
  public :: define_DI
  public :: a3_bc_dirichlet_mirror
  public :: a3_bc_dirichlet_zero_fc
  public :: a3_bc2_dirichlet_mirror
...
contains
  ...
  subroutine init_cond_initialize(cfg, n_dim)
    ...
    call CFG_add(cfg, "seed_rel_r0", [0.5_dp, 0.5_dp, 0.5_dp], &
         "The relative start position of the initial seed", .true.)
    call CFG_add(cfg, "seed_rel_r1", [0.5_dp, 0.5_dp, 0.5_dp], &
    ...
    call CFG_add(cfg, "die_rel_center",[0.5_dp, 0.5_dp, 0.5_dp], &
         "Relative central position of the dielectric", .true.)
    call CFG_add(cfg, "die_rel_axis",[0.5_dp, 0.5_dp, 0.5_dp], &
  ...
  subroutine init_cond_stochastic_density(tree)
    use m_a3_ghostcell, only: a3_bc_neumann_zero
    use m_a3_prolong
    ...
    call a3_tree_clear_cc(tree, i_rhs)
    !$omp do private(id)
    ...
    do lvl = my_lvl, tree%highest_lvl-1
       do i = 1, size(tree%lvls(lvl)%parents)
          ...
          call a3_gc_box(tree%boxes, id, i_eps, i_rhs, &
               a3_gc_interp, a3_bc_neumann_zero)
       end do

       do i = 1, size(tree%lvls(lvl)%parents)
          id = tree%lvls(lvl)%parents(i)
          call a3_prolong_linear_from(tree%boxes, id, i_eps, i_rhs, add=.true.)
       end do
    ...
    do lvl = my_lvl, tree%highest_lvl
       do i = 1, size(tree%lvls(lvl)%ids)
          ...
          call a3_box_add_cc(tree%boxes(id), i_rhs, i_electron)
          call a3_box_add_cc(tree%boxes(id), i_rhs, i_pos_ion)
       end do
    end do
    ...
    call a3_gc_tree(tree, i_eps, i_electron, a3_gc_interp_lim, a3_bc_neumann_zero)
    call a3_gc_tree(tree, i_eps, i_pos_ion, a3_gc_interp_lim, a3_bc_neumann_zero)
    ...
    do k = 0, nc+1; do j = 0, nc+1; do i = 0, nc+1
       ...
       do n = 1, init_conds%n_cond
          ...
          if (init_conds%seed_charge_type(n) <= 0) then
             box%cc(i, j, k, i_electron) = box%cc(i, j, k, i_electron) + density
          end if

          if (init_conds%seed_charge_type(n) >= 0) then
             box%cc(i, j, k, i_pos_ion) = box%cc(i, j, k, i_pos_ion) + density
          end if
       end do
    end do; end do; end do
  ...
  subroutine define_DI(box)
    ...
    do k = 0,  nc+1; do j = 0,  nc+1; do i = 0,  nc+1
       ...
       do n = 1, init_conds%n_die
         if(DI_interior(init_conds%die_type(n), init_conds%die_center(:,n), &
            init_conds%die_axis(:,n), 3,rr(:))) then
               box%cc(i, j, k, i_eps)        = a3_harm_w(1.0_dp, ST_epsilon_die, f)
               box%cc(i, j, k, i_electron)   = init_conds%die_density(n)
               box%cc(i, j, k, i_pos_ion)    = init_conds%die_density(n)
         end if
       end do
    end do; end do; end do
    ...
    if (maxval(box%cc(:, :, :, i_eps)) > minval(box%cc(:, :, :, i_eps))) then  
      ...
      do k = 0,  nc+1; do j = 0,  nc+1; do i = 0,  nc+1
         ...
         if(maxval([i, j, k]) < nc+1 .and. minval([i, j, k])> 0) then
  ...
  real(dp) function vol_frac_inside(box, ix)
    ...
    do k = 1,  8; do j = 1,  8; do i = 1,  8
      ...
      r(3) = rr(3) + k*dr/8.0d0 - dr/2.0d0 - dr/16.0d0
        ...
        do n = 1, init_conds%n_die
          if (DI_interior(init_conds%die_type(n), init_conds%die_center(:,n), &
            init_conds%die_axis(:,n), 3, r(:))) then
              in_counts=in_counts+1.0_dp
          end if
        end do
    end do; end do; end do
    
    vol_frac_inside = in_counts / (8.0_dp)**3
  ...
  subroutine a3_bc_dirichlet_mirror(box, nb, iv, bc_type, i_eps)
    type(box3_t), intent(inout) :: box
    ...
    call a3_set_box_gc(box, nb, iv, init_conds%background_density)

  end subroutine a3_bc_dirichlet_mirror
  
  subroutine a3_bc2_dirichlet_mirror(boxes, id, nb, i_eps, iv, gc_side, nc, med)
  ...
  end subroutine a3_bc2_dirichlet_mirror
  
  subroutine a3_bc_dirichlet_zero_fc(box, nb, s_iv, bc_type, i_eps)
    type(box3_t), intent(inout) :: box
    ...
    bc_type = af_bc_dirichlet
    call a3_set_box_gc_fc(box, nb, s_iv, gc_scalar = [0.0_dp, 0.0_dp, 0.0_dp])

  end subroutine a3_bc_dirichlet_zero_fc
  
  

end module m_init_cond_3d
