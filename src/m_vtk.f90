...
contains
  ...
  subroutine vtk_ini_xml(vtkf, filename, vtk_type)
    ...
    ! open(newunit=vtkf%sunit, form='UNFORMATTED', access='STREAM', &
  ...
  subroutine vtk_geo_xml(vtkf, n_nodes, n_cells, coords, n_dim)
    type(vtk_t), intent(inout) :: vtkf
    integer, intent(in) :: n_nodes, n_cells, n_dim
    real(dp), intent(in) :: coords(:)
    character(len=buf_len) :: bufstr
    integer :: n_bytes

    if (n_dim /= 3) stop "n_dim should be 3 :("
    ...
    write(bufstr, fmt="(A,I0,A,I0,A)") repeat(' ',vtkf%indent) // &
         '<DataArray type="Float64" NumberOfComponents="', n_dim, &
         '" Name="Points" format="appended" offset="', &
         vtkf%offset, '"/>'
    write(vtkf%funit) trim(bufstr) // endl
    
    ! Write raw data
    n_bytes = n_dim * n_nodes * 8
    vtkf%offset = vtkf%offset + 4 + n_bytes ! 4 for integer
    write(vtkf%sunit) n_bytes
    write(vtkf%sunit) coords
  ...
  subroutine vtk_con_xml(vtkf, n_cells, connects, offsets, cell_types)
    type(vtk_t), intent(inout) :: vtkf
    ...
    n_bytes = offsets(n_cells) * 4
    vtkf%offset = vtkf%offset + 4 + n_bytes
  ...
  subroutine vtk_dat_xml(vtkf, xml_name, true_is_open)
    type(vtk_t), intent(inout) :: vtkf
    character(*), intent(IN)::           xml_name     !< Location of saving variables: CELL or NODE centered.
