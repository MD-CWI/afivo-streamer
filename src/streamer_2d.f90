...
program streamer_2d
  ...
  do it = 1, huge(1)-1
     ...
     do i = 1, 2
        ...
        ! Update the solution
        call a2_loop_box_arg(tree, update_solution, [ST_dt, real(i, dp)], .true.)
        ...
        if (i == 1) then
           ...
           call a2_restrict_tree(tree, i_electron, i_eps = i_eps, med = med)
           call a2_restrict_tree(tree, sigma_rhs, i_eps = i_eps, med = med, s_flag = .true.)
        end if
     end do
     ...
     if (write_out) then
        ...
        call a2_gc_tree(tree, i_eps, i_electron, a2_gc_interp, a2_bc_dirichlet_mirror)
        call a2_gc_tree(tree, i_eps, i_pos_ion, a2_gc_interp, a2_bc_dirichlet_mirror)

        if (ST_output_src_term) then
           call a2_restrict_tree(tree, i_src, i_eps = i_eps, med = med)
           call a2_gc_tree(tree, i_eps, i_src, a2_gc_interp, a2_bc_neumann_zero)
        end if

        if (photoi_enabled) then
           call photoi_set_src(tree, ST_dt) ! Because the mesh could have changed
           call a2_restrict_tree(tree, i_photo, i_eps = i_eps, med = med)
           call a2_gc_tree(tree, i_eps, i_photo, a2_gc_interp, a2_bc_neumann_zero)
        end if
        ...
        if (ST_datfile_write) then
           call a2_write_tree(tree, fname, ST_output_dir)
        end if
        ...
        if (ST_plane_write) then
           write(fname, "(A,I6.6)") trim(ST_simulation_name) // &
                "_plane_", output_cnt
           call a2_write_plane(tree, fname, [ST_plane_ivar], &
        ...
        if (ST_lineout_write) then
           write(fname, "(A,I6.6)") trim(ST_simulation_name) // &
                "_line_", output_cnt
           call a2_write_line(tree, trim(fname), &
                [i_electron, i_pos_ion, i_phi, i_electric_fld, i_eps], &
                r_min = ST_lineout_rmin(1:2) * ST_domain_len, &
                r_max = ST_lineout_rmax(1:2) * ST_domain_len, &
     ...
     if (mod(it, ST_refine_per_steps) == 0) then
        ...
        call a2_restrict_tree(tree, i_electron, i_eps = i_eps, med = med)
        call a2_restrict_tree(tree, i_pos_ion, i_eps = i_eps, med = med)
        call a2_restrict_tree(tree, sigma_rhs, i_eps = i_eps, med = med, s_flag = .true.)
...
contains
  ...
  subroutine refine_routine(box, cell_flags)
    ...
    real(dp)                 :: dist, rmin(2), rmax(2), grad_n(2), Efld(2)
    ...
    do j = 1, nc; do i = 1, nc
       ...
       cphi = dx2 * abs(box%cc(i, j, i_rhs)) / ST_refine_cphi
       
       

         grad_n(1) = 0.5_dp*(gradient_2d(box, 1, [i, j], 1, i_electron) + &
                     gradient_2d(box, 1, [i, j], 2, i_electron))
         grad_n(2) = 0.5_dp*(gradient_2d(box, 1, [i, j], 3, i_electron) + &
                     gradient_2d(box, 1, [i, j], 4, i_electron))

         Efld(1)   = 0.5_dp*(gradient_2d(box, 2, [i, j], 1, i_phi) + &
                     gradient_2d(box, 2, [i, j], 2, i_phi))
         Efld(2)   = 0.5_dp*(gradient_2d(box, 2, [i, j], 3, i_phi) + &
                     gradient_2d(box, 2, [i, j], 4, i_phi))
  ...
  subroutine fluxes_koren(boxes, id)
    ...
    allocate(cc(-1:nc+2, -1:nc+2))
    allocate(eps(-1:nc+2, -1:nc+2))
    allocate(sigma(0:nc+2, 0:nc+2, 2))

    ! Fill ghost cells on the sides of boxes (no corners)
    call a2_gc_box(boxes, id, i_eps, i_electron, a2_gc_interp, &
         a2_bc_dirichlet_mirror, .false.)
         
    call a2_gc_box_fc(boxes, id, i_eps, sigma_rhs, a2_gc_prolong_copy_fc, &
         a2_bc_dirichlet_zero_fc, med)
         
    call a2_gc2_box(boxes, id, i_eps, i_electron, a2_gc2_prolong_linear, &
         a2_bc2_dirichlet_mirror, cc, nc, med)
    ...
    do j = 1, nc+1; do i = 1, nc+1  
      ...
      fld = abs(boxes(id)%fc(i, j, 1, electric_fld))
      v(i, j, 1) = -LT_get_col(ST_td_tbl, i_mobility, fld) * &
           boxes(id)%fc(i, j, 1, electric_fld)
      dc(i, j, 1) = LT_get_col(ST_td_tbl, i_diffusion, fld)
      
      fld = abs(boxes(id)%fc(i, j, 2, electric_fld))
      v(i, j, 2) = -LT_get_col(ST_td_tbl, i_mobility, fld) * &
           boxes(id)%fc(i, j, 2, electric_fld)
      dc(i, j, 2) = LT_get_col(ST_td_tbl, i_diffusion, fld)

    end do; end do

    call flux_koren_2d(cc, v, nc, inv_dr)
    call flux_diff_2d(boxes(id), dc, i_electron)
    ...
    if (ST_update_ions) then
       ...
       v(:, :, 1:2) = ST_ion_mobility * &
            boxes(id)%fc(:, :, 1:2, electric_fld)

       ! Fill ghost cells on the sides of boxes (no corners)
       call a2_gc_box(boxes, id, i_eps, i_pos_ion, a2_gc_interp, &
            a2_bc_dirichlet_mirror, .false.)

       call flux_koren_2d(cc, v, nc, inv_dr)
       call flux_diff_2d(boxes(id), dc, i_pos_ion)
  ...
  subroutine update_solution(box, args)
    ...
    real(dp)                     :: dt, inv_dr, src, fld, fld_vec(2), s_flow(2)
    ...
    do j = 1, nc; do i = 1, nc
       ...
       alpha = LT_get_col_at_loc(ST_td_tbl, i_alpha, loc)*a2_heaviside(box%cc(i, j, i_eps), med)
       eta   = LT_get_col_at_loc(ST_td_tbl, i_eta, loc)*a2_heaviside(box%cc(i, j, i_eps), med)
       ...
       if (box%cc(i, j, i_eps) <= med) then
         ...
         if (box%cc(i-1, j, i_eps) > med) then
           s_flow(1) = dt * (rfac(1) * box%fc(i, j, 1, flux_elec) - &
               rfac(2) * box%fc(i+1, j, 1, flux_elec)) 
           box%fc(i, j, 1, sigma_rhs) = box%fc(i, j, 1, sigma_rhs) + fac * s_flow(1) 
         end if
         if (box%cc(i, j-1, i_eps) > med) then
           s_flow(2) = dt * (box%fc(i, j, 2, flux_elec) - &
               box%fc(i, j+1, 2, flux_elec)) 
           box%fc(i, j, 2, sigma_rhs) = box%fc(i, j, 2, sigma_rhs) + fac * s_flow(2) 
         end if  
       ...
       if (a2_border(box%cc(i, j, i_eps), box%cc(i-1, j, i_eps), med) .and. &
          a2_border(box%cc(i, j, i_eps), box%cc(i, j-1, i_eps), med) .and. &
          box%fc(i, j, 2, sigma_rhs) >= box%fc(i, j, 1, sigma_rhs)) then
         s_fac = 0.25_dp*box%fc(i, j, 2, sigma_rhs) * (box%fc(i, j, 2, sigma_rhs) - box%fc(i, j, 1, sigma_rhs)) / &
               (box%fc(i, j, 2, sigma_rhs) + box%fc(i, j, 1, sigma_rhs) + epsilon(1.0_dp))
         box%fc(i, j, 1, sigma_rhs) = box%fc(i, j, 1, sigma_rhs) + s_fac
         box%fc(i, j, 2, sigma_rhs) = box%fc(i, j, 2, sigma_rhs) - s_fac
         else if(a2_border(box%cc(i, j, i_eps), box%cc(i-1, j, i_eps), med) .and. &
          a2_border(box%cc(i, j, i_eps), box%cc(i, j-1, i_eps), med)) then
         s_fac = 0.25_dp*box%fc(i, j, 1, sigma_rhs) * (box%fc(i, j, 1, sigma_rhs) - box%fc(i, j, 2, sigma_rhs)) / &
               (box%fc(i, j, 2, sigma_rhs) + box%fc(i, j, 1, sigma_rhs) + epsilon(1.0_dp))
         box%fc(i, j, 1, sigma_rhs) = box%fc(i, j, 1, sigma_rhs) - s_fac
         box%fc(i, j, 2, sigma_rhs) = box%fc(i, j, 2, sigma_rhs) + s_fac
       end if
       ...
       ! Source term
       src = fld * mu * box%cc(i, j, i_electron) * (alpha - eta)
       if (photoi_enabled .and. box%cc(i, j, i_eps) <= med) src = src + box%cc(i, j, i_photo)

       if (i_step == 1 .and. ST_output_src_term) then
          if (ST_output_src_decay_rate < 0) then
             ! No time-averaging
             box%cc(i, j, i_src) = src
          else
             ! Approximate exp(-t*rate) as 1-t*rate
             box%cc(i, j, i_src) = (1 - dt * ST_output_src_decay_rate) * &
                  box%cc(i, j, i_src) + src * dt
          end if
       end if
       ...
       if (box%cc(i, j, i_eps) <= med) then
         box%cc(i, j, i_electron) = box%cc(i, j, i_electron) + f_elec + src
       
         ! Add flux and source term
         box%cc(i, j, i_pos_ion)  = box%cc(i, j, i_pos_ion) + f_ion + src
       end if

       ! Possible determine new time step restriction
       if (i_step == 2) then

          fld_vec(1) = 0.5_dp * (box%fc(i, j, 1, electric_fld) + &
               box%fc(i+1, j, 1, electric_fld))
          fld_vec(2) = 0.5_dp * (box%fc(i, j, 2, electric_fld) + &
          ...
          ! Diffusion condition
          dt_dif = box%dr**2 / max(2 * 2 * max(diff, ST_ion_diffusion), epsilon(1.0_dp))
          ! Dielectric relaxation time
          dt_drt = UC_eps0 / (UC_elem_charge * (mu + abs(ST_ion_mobility)) * &
               max(box%cc(i, j, i_electron), epsilon(1.0_dp)))
  ...
  subroutine prolong_to_new_boxes(tree, ref_info)
    ...
    do lvl = 1, tree%highest_lvl
       ...
       do i = 1, size(ref_info%lvls(lvl)%add)
          ...
          call a2_prolong_linear(tree%boxes(p_id), tree%boxes(id), i_electron, i_eps = i_eps, med = med)
          call a2_prolong_linear(tree%boxes(p_id), tree%boxes(id), i_pos_ion, i_eps = i_eps, med = med)
          call a2_prolong_sparse(tree%boxes(p_id), tree%boxes(id), i_phi, i_eps = i_eps)
          if (photoi_enabled) then
             call a2_prolong_linear(tree%boxes(p_id), tree%boxes(id), i_photo, i_eps = i_eps, med = med)
          end if
          if (ST_output_src_term) then
             call a2_prolong_linear(tree%boxes(p_id), tree%boxes(id), i_src, i_eps = i_eps, med = med)
          end if
       ...
       do i = 1, size(ref_info%lvls(lvl)%add)
          id = ref_info%lvls(lvl)%add(i)
          call a2_gc_box(tree%boxes, id, i_eps, i_electron, &
               a2_gc_interp, a2_bc_dirichlet_mirror)
          call a2_gc_box(tree%boxes, id, i_eps, i_pos_ion, &
               a2_gc_interp, a2_bc_dirichlet_mirror)
          call a2_gc_box(tree%boxes, id, i_eps, i_phi, &
               mg%sides_rb, mg%sides_bc)
          if (photoi_enabled) then
             call a2_gc_box(tree%boxes, id, i_eps, i_photo, &
               a2_gc_interp, photoi_helmh_bc)
          end if
          if (ST_output_src_term) then
             call a2_gc_box(tree%boxes, id, i_eps, i_src, &
  ...
  subroutine write_log_file(tree, filename, out_cnt, dir)
    ...
    real(dp)                     :: velocity, dt
    real(dp), save               :: prev_pos(2) = 0
    real(dp)                     :: sum_elec, sum_pos_ion
    real(dp)                     :: max_elec, max_field, max_Er
    type(a2_loc_t)              :: loc_elec, loc_field, loc_Er

    call a2_prepend_directory(filename, dir, fname)

    call a2_tree_sum_cc(tree, i_electron, sum_elec)
    call a2_tree_sum_cc(tree, i_pos_ion, sum_pos_ion)
    call a2_tree_max_cc(tree, i_electron, max_elec, loc_elec)
    call a2_tree_max_cc(tree, i_electric_fld, max_field, loc_field)
    call a2_tree_max_fc(tree, 1, electric_fld, max_Er, loc_Er)
    ...
    if (out_cnt == 1) then
       ...
       prev_pos = a2_r_loc(tree, loc_field)
    end if

    velocity = norm2(a2_r_loc(tree, loc_field) - prev_pos) / ST_dt_output
    prev_pos = a2_r_loc(tree, loc_field)
    ...
    write(my_unit, fmt) out_cnt, ST_time, dt, velocity, sum_elec, &
         sum_pos_ion, max_field, a2_r_loc(tree, loc_field), max_elec, &
         a2_r_loc(tree, loc_elec), max_Er, a2_r_loc(tree, loc_Er), &
         wc_time, a2_num_cells_used(tree)
