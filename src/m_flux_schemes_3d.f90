...
contains
  ...
  !> Compute diffusive flux (second order)
  subroutine flux_diff_3d(box, dc, iv)
    type(box3_t), intent(in)  :: box
    integer, intent(in)        :: iv
    real(dp), intent(inout)    :: dc(1:box%n_cell+1, 1:box%n_cell+1, 1:box%n_cell+1, 3)
    integer                    :: nc, d, i, j, k
    ...
    do k = 1,  nc+1; do j = 1,  nc+1; do i = 1,  nc+1
       do d = 1, 3  
          dc(i, j, k, d) = - dc(i, j, k, d) * gradient_3d(box, 1, [i, j, k], 2*d-1, iv)
       end do
  ...
  function gradient_3d(box, cc_num, ix, dir, iv) result(grad)
    type(box3_t), intent(in)         :: box
    integer, intent(in)               :: cc_num !< 2 if cc is assumed continuous 1 otherwise
    integer, intent(in)               :: ix(3), dir, iv
    real(dp)                          :: grad, inv_dr, s_C, harm_ep, med, fac
    integer                           :: n_ix(3)
    
    n_ix      = a3_neighb_dix(:, dir)
    inv_dr    = 1/box%dr
    med       = a3_harm_w(1.0_dp, ST_epsilon_die, 0.5_dp)
 

    if (a3_border(box%cc(ix(1), ix(2), ix(3), i_eps), box%cc(ix(1)+n_ix(1), ix(2)+n_ix(2), ix(3)+n_ix(3), i_eps), med)) then
      if (cc_num == 2) then
