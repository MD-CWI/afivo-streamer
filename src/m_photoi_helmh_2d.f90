...
module m_photoi_helmh_2d
  ...
  type(mg2_t) :: mg_helm       ! Multigrid option struct
...
contains
  ...
  subroutine photoi_helmh_initialize(cfg, is_used)
    ...
    if (is_used) then
       ...
       mg_helm%sides_bc => a2_bc_dirichlet_zero
  ...
  subroutine photoi_helmh_compute(tree)
    ...
    call a2_tree_clear_cc(tree, i_photo)
    ...
    do n = 1, n_modes
       ...
       if (.not. have_guess) then
          call mg2_fas_fmg(tree, mg_helm, .false., have_guess)
       else
          have_guess = .true.
          do i = 1, num_vcycles
             call mg2_fas_vcycle(tree, mg_helm, .false.)
          end do
       end if
  ...
  subroutine photoi_helmh_bc(box, nb, iv, bc_type, i_eps)
    ...
    select case (nb)
    case (a2_neighb_lowx, a2_neighb_highx)
       call a2_bc_neumann_zero(box, nb, iv, bc_type)

    case (a2_neighb_lowy, a2_neighb_highy)
       call a2_bc_dirichlet_zero(box, nb, iv, bc_type)
