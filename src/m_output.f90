...
!     end if
  ...
  subroutine output_etc(tree, filename, out_cnt, wc_time)
    ...
    integer                      :: my_unit
    real(dp)                     :: maxE, maxE_e, maxE_rhs
    real(dp)                     :: azone_Efield, azone_e, azone_rhs
    real(dp)                     :: scl_Efield, scl_e, scl_rhs
    real(dp)                     :: bhnd_Efield, bhnd_e, bhnd_rhs
    ...
    if (out_cnt == 1) then
      open(newunit = my_unit, file=trim(filename), action="write")
       write(my_unit, "(A)", advance="no") "time max(E) x y e rhs &
            &active_zone_x y elec_field e rhs space_charge_x y &
            &elec_field e rhs behind_x y elec_field e rhs"
       write(my_unit, *) ""
       close(my_unit)
    end if
    ...
    open(newunit=my_unit, file=trim(filename), action="write", &
         position="append")
    write(my_unit, fmt) global_time, maxE, af_r_loc(tree, loc_maxE), maxE_e, maxE_rhs, &
         af_r_loc(tree, loc_azone), azone_Efield, azone_e, azone_rhs, &
         af_r_loc(tree, loc_scl), scl_Efield, scl_e, scl_rhs, &
         af_r_loc(tree, loc_bhnd), bhnd_Efield, bhnd_e, bhnd_rhs
    close(my_unit)
#endif
