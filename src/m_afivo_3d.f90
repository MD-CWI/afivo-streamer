...
module m_afivo_3d
  ...
  type box3_t
     ...
     integer               :: ix(3)          ! index in the domain
     integer               :: parent         ! index of parent in box list
     integer               :: children(8)    ! index of children in box list
     integer               :: neighbors(6)   ! index of neighbors in box list
     integer               :: n_cell         ! number of cells per dimension
     real(dp)              :: dr             ! width/height of a cell
     real(dp)              :: r_min(3)       ! min coords. of box
  ...
  type a3_t
     ...
     real(dp)                  :: r_base(3)  ! coords of box at index (1,1,1)
     real(dp)                  :: dr_base    ! cell spacing at lvl 1
     type(lvl3_t), allocatable :: lvls(:)    ! list storing the tree levels
     type(box3_t), allocatable :: boxes(:)   ! list of all boxes
  end type a3_t

  abstract interface
     integer function a3_to_int_f(box)
       import
       type(box3_t), intent(in) :: box
     end function a3_to_int_f

     subroutine a3_subr(box)
       import
       type(box3_t), intent(inout) :: box
     end subroutine a3_subr

     subroutine a3_subr_arg(box, rarg)
       import
       type(box3_t), intent(inout) :: box
       real(dp), intent(in)        :: rarg(:)
     end subroutine a3_subr_arg

     subroutine a3_subr_boxes(boxes, id)
       import
       type(box3_t), intent(inout) :: boxes(:)
       integer, intent(in)         :: id
     end subroutine a3_subr_boxes

     subroutine a3_subr_gc(boxes, id, i, ivs)
       import
       type(box3_t), intent(inout) :: boxes(:)
       integer, intent(in)         :: id, i, ivs(:)
     end subroutine a3_subr_gc
  end interface

contains

  ! Initialize a 3d tree type
  subroutine a3_init(tree, max_lvl, n_boxes, n_cell, n_var_cell, n_var_face, &
       dr, r_min)
    type(a3_t), intent(out) :: tree       ! The tree to initialize
    ...
    real(dp), intent(in)    :: r_min(3)   ! Lower left coordinate of box 1,1
    integer                 :: lvl

    if (n_cell < 2)       stop "a3_init: n_cell should be >= 2"
    if (btest(n_cell, 0)) stop "a3_init: n_cell should be even"
    if (n_var_cell <= 0)  stop "a3_init: n_var_cell should be > 0"
    if (n_boxes <= 0)     stop "a3_init: n_boxes should be > 0"
  ...
  subroutine a3_destroy(tree)
    ...
    tree%max_id = 0
  end subroutine a3_destroy

  ! Call procedure for each box in tree
  subroutine a3_loop_box(tree, my_procedure)
    type(a3_t), intent(inout) :: tree
    procedure(a3_subr)        :: my_procedure
    integer                   :: lvl, i, id
  ...
  subroutine a3_loop_box_arg(tree, my_procedure, rarg)
    type(a3_t), intent(inout) :: tree
    procedure(a3_subr_arg)    :: my_procedure
    real(dp), intent(in)      :: rarg(:)
    integer                   :: lvl, i, id
  ...
  subroutine a3_loop_boxes(tree, my_procedure)
    type(a3_t), intent(inout) :: tree
    procedure(a3_subr_boxes)  :: my_procedure
    integer                   :: lvl, i, id
  ...
  subroutine a3_tidy_up(tree, max_frac_used, goal_frac_used, &
    ...
    if (goal_frac_used > max_frac_used) &
         stop "a3_tidy_up: need goal_frac_used < max_frac_used"
    if (max_frac_used > 1.0_dp) stop "a3_tidy_up: need max_frac_used < 1"
    if (n_clean_min < 1)        stop "a3_tidy_up: need n_clean_min > 0"
    ...
    if (new_size /= old_size .or. only_reorder) then
       print *, "a3_tidy_up:", old_size, new_size, only_reorder
  ...
  subroutine set_leaves_parents(boxes, level)
    ...
    type(lvl3_t), intent(inout) :: level
    ...
    do i = 1, size(level%ids)
       id = level%ids(i)
       if (a3_has_children(boxes(id))) then
          i_parent                = i_parent + 1
  ...
  subroutine set_nbs_3d(boxes, id)
    type(box3_t), intent(inout) :: boxes(:)
    integer, intent(in)          :: id
    integer                      :: nb, nb_id
    ...
    do nb = 1, 6
       if (boxes(id)%neighbors(nb) == a5_no_box) then
          nb_id = find_nb_3d(boxes, id, nb)
          if (nb_id > a5_no_box) then
             boxes(id)%neighbors(nb) = nb_id
             boxes(nb_id)%neighbors(a3_nb_rev(nb)) = id
          end if
       end if
    end do
  end subroutine set_nbs_3d

  ! Compute the child index of ix
  integer function a3_ix_to_cix(ix)
    integer, intent(in) :: ix(3)
    ! third index odd: -4, second index odd: -2, first ix odd: -1
    a3_ix_to_cix = 8 - 4 * iand(ix(3), 1) - 2 * iand(ix(2), 1) - iand(ix(1), 1)
  end function a3_ix_to_cix

  ! Get neighbor nb of id, through its parent
  function find_nb_3d(boxes, id, nb) result(nb_id)
    type(box3_t), intent(in) :: boxes(:)
  ...
  subroutine a3_set_base(tree, ix_list, nb_list)
    ...
    do i = 1, n_boxes
       do nb = 1, 6
          nb_id = nb_list(nb, i)
          if (nb_id > a5_no_box) nb_list(a3_nb_rev(nb), nb_id) = i
       end do
    end do

    if (any(nb_list == a5_no_box)) stop "a3_set_base: unresolved neighbors"
    ...
    call a3_get_free_ids(tree, tree%lvls(1)%ids)
    tree%n_lvls = 1
  ...
  subroutine a3_adjust_refinement(tree, ref_func, n_changes)
    type(a3_t), intent(inout)      :: tree
    procedure(a3_to_int_f)         :: ref_func
    integer, intent(out), optional :: n_changes
    integer                        :: lvl, id, i, c_ids(8), i_c
    ...
    call a3_clear_tagbit(tree, a5_bit_new_children)

    ! Set refinement values for all boxes
    call a3_set_ref_flags(tree%lvls, tree%n_lvls, tree%max_lvl, tree%boxes, &
         ref_flags, ref_func)
    ...
    do lvl = 1, tree%max_lvl-1
       do i = 1, size(tree%lvls(lvl)%ids)
          ...
          if (id > max_id_prev) then
             ...
             ! Add children. First need to get 8 free id's
             call a3_get_free_ids(tree, c_ids)
             call a3_add_children(tree%boxes, id, c_ids)
       ...
       do i = 1, size(tree%lvls(lvl)%ids)
          ...
          if (a3_has_children(tree%boxes(id))) then
             do i_c = 1, 8
                call set_nbs_3d(tree%boxes, tree%boxes(id)%children(i_c))
             end do
          end if
       end do
    ...
    do lvl = 1, tree%n_lvls
       n_parents = size(tree%lvls(lvl+1)%ids)/8
  ...
  subroutine a3_get_free_ids(tree, ids)
    type(a3_t), intent(inout) :: tree
  ...
  end subroutine a3_get_free_ids

  ! Given the refinement function, return consistent refinement flags
  subroutine a3_set_ref_flags(lvls, n_lvls, max_lvl, boxes, ref_flags, ref_func)
    type(lvl3_t), intent(in)    :: lvls(:)
    type(box3_t), intent(inout) :: boxes(:)
    integer, intent(in)         :: n_lvls, max_lvl
    integer, intent(inout)      :: ref_flags(:)
    procedure(a3_to_int_f)      :: ref_func
    integer                     :: lvl, i, id, c_ids(8)
       ...
       do i = 1, size(lvls(lvl)%leaves) ! We only check leaf boxes
          ...
          if (ref_flags(id) == a5_do_ref) then
             ...
             do nb = 1, 6
                nb_id = boxes(id)%neighbors(nb)
                if (nb_id > a5_no_box) then
                   if (a3_has_children(boxes(nb_id)) .or. &
  ...
  subroutine a3_remove_children(boxes, id)
    type(box3_t), intent(inout) :: boxes(:)
    integer, intent(in)         :: id
    ...
    do ic = 1, 8
       ...
       do nb = 1, 6             ! Remove from neighbors
  ...
  end subroutine a3_remove_children

  ! Add children to box id, using the indices in c_ids
  subroutine a3_add_children(boxes, id, c_ids)
    type(box3_t), intent(inout) :: boxes(:)
    integer, intent(in)         :: id, c_ids(8)
    integer                     :: i, nb, ch_nb(4), c_id, c_ix_base(3)
    ...
    do i = 1, 8
       c_id                  = c_ids(i)
       boxes(c_id)%ix        = c_ix_base + a3_ch_dix(:,i)
  ...
  subroutine set_child_ids(p_ids, c_ids, boxes)
    ...
    ! Count 8 times the number of refined parent blocks
    n_children = 8 * count(a3_has_children(boxes(p_ids)))
    allocate(c_ids(n_children))

    i_c = 0
    do i = 1, size(p_ids)
       ip = p_ids(i)
       if (a3_has_children(boxes(ip))) then
          c_ids(i_c+1:i_c+8) = boxes(ip)%children
          i_c = i_c + 8
       end if
    end do
  ...
  elemental logical function a3_has_children(box)
    type(box3_t), intent(in) :: box
    a3_has_children = (box%children(1) /= a5_no_box)
  end function a3_has_children

  ! Return minimum dr that is used in the tree
  pure function a3_min_dr(tree) result(dr)
    type(a3_t), intent(in) :: tree
    real(dp)               :: dr(3)
    dr = tree%dr_base * 0.5_dp**(tree%n_lvls-1)
  end function a3_min_dr

  ! Return the coordinate of the center of a box
  pure function a3_r_center(box) result(r_center)
    type(box3_t), intent(in) :: box
    real(dp)                 :: r_center(3)
    r_center = box%r_min + 0.5_dp * box%n_cell * box%dr
  end function a3_r_center

  ! Location of cell center with index cc_ix
  pure function a3_r_cc(box, cc_ix) result(r)
    type(box3_t), intent(in) :: box
    integer, intent(in)      :: cc_ix(3)
    real(dp)                 :: r(3)
    r = box%r_min + (cc_ix-0.5_dp) * box%dr
  end function a3_r_cc

  ! Location of node with index nd_ix
  pure function a3_r_node(box, nd_ix) result(r)
    type(box3_t), intent(in) :: box
    integer, intent(in)      :: nd_ix(3)
    real(dp)                 :: r(3)
    r = box%r_min + (nd_ix-1) * box%dr
  end function a3_r_node
  ...
  subroutine a3_box_add_cc(box, iv_from, iv_to)
    type(box3_t), intent(inout) :: box
    integer, intent(in)         :: iv_from, iv_to
    box%cc(:,:,:, iv_to) = box%cc(:,:,:, iv_to) + box%cc(:,:,:, iv_from)
  end subroutine a3_box_add_cc

  ! Subtract cc(:,:,:, iv_from) from box%cc(:,:,:, iv_to)
  subroutine a3_box_sub_cc(box, iv_from, iv_to)
    type(box3_t), intent(inout) :: box
    integer, intent(in)         :: iv_from, iv_to
    box%cc(:,:,:, iv_to) = box%cc(:,:,:, iv_to) - box%cc(:,:,:, iv_from)
  end subroutine a3_box_sub_cc

  ! Copy cc(:,:,:, iv_from) to box%cc(:,:,:, iv_to)
  subroutine a3_box_copy_cc(box, iv_from, iv_to)
    type(box3_t), intent(inout) :: box
    integer, intent(in)         :: iv_from, iv_to
    box%cc(:,:,:, iv_to) = box%cc(:,:,:, iv_from)
  end subroutine a3_box_copy_cc

  ! Copy cc(:,:,:, iv_from) to box%cc(:,:,:, iv_to) for all ids
  subroutine a3_boxes_copy_cc(boxes, ids, iv_from, iv_to)
    type(box3_t), intent(inout) :: boxes(:)
    ...
    !$omp do
    do i = 1, size(ids)
       call a3_box_copy_cc(boxes(ids(i)), iv_from, iv_to)
    end do
    !$omp end do
  end subroutine a3_boxes_copy_cc

  ! Injection to children (zero order)
  subroutine a3_prolong0_from(boxes, id, ivs, fill_gc)
    type(box3_t), intent(inout) :: boxes(:)
    integer, intent(in)         :: id, ivs(:)
    logical, intent(in)         :: fill_gc
    integer                     :: nc, i_c, c_id, ix_offset(3)
    integer                     :: i, j, k, i_c1, j_c1, k_c1, iv, v

    nc = boxes(id)%n_cell
    do i_c = 1, 8
       c_id = boxes(id)%children(i_c)
       ix_offset = a3_ch_dix(:, i_c) * ishft(nc, -1) ! Offset child

       do v = 1, size(ivs)
          iv = ivs(v)
          do k = 1-l2i(fill_gc), nc+l2i(fill_gc)
             k_c1 = ix_offset(3) + ishft(k+1, -1) ! (j+1)/2
  ...
  end subroutine a3_prolong0_from
  ...
  ! Conservative restriction. 8 fine cells to one parent cell
  subroutine a3_restrict_to_box(boxes, id, ivs)
    type(box3_t), intent(inout) :: boxes(:)
    integer, intent(in)         :: id, ivs(:)
    integer                     :: nc, i_c, c_id, ix_offset(3)
    ...
    do i_c = 1, 8
       ...
       ! Offset of child w.r.t. parent
       ix_offset = a3_ch_dix(:, i_c) * ishft(nc, -1)
       call a3_restrict_box(boxes(c_id), boxes(id), ix_offset, ivs)
    end do
  end subroutine a3_restrict_to_box

  ! Restrict variables ivs(:) to all parent boxes ids(:)
  subroutine a3_restrict_to_boxes(boxes, ids, ivs)
    type(box3_t), intent(inout) :: boxes(:)
    ...
    !$omp do
    do i = 1, size(ids)
       call a3_restrict_to_box(boxes, ids(i), ivs)
    end do
    !$omp end do
  ...
  subroutine a3_gc_sides(tree, ivs, subr_no_nb, subr_bc)
    type(a3_t), intent(inout) :: tree
    integer, intent(in)       :: ivs(:)
    procedure(a3_subr_gc)     :: subr_no_nb, subr_bc
    integer                   :: lvl, i, id
  ...
  end subroutine a3_gc_sides

  ! Fill ghost cells for variables ivs(:) on the sides of a boxes, using
  ! subr_no_nb on refinement boundaries and subr_bc on physical boundaries
  subroutine a3_gc_box_sides(boxes, id, ivs, subr_no_nb, subr_bc)
    type(box3_t), intent(inout) :: boxes(:)
    integer, intent(in)         :: id, ivs(:)
    procedure(a3_subr_gc)       :: subr_no_nb, subr_bc
    integer                     :: nb
    ...
    do nb = 1, 6
       if (boxes(id)%neighbors(nb) > a5_no_box) then
          call a3_gc_side_from_nb(boxes, id, nb, ivs)
  ...
  end subroutine a3_gc_box_sides
  ...
  ! of its neighbors (nb).
  subroutine a3_get_cc_from_nb(boxes, id, nb, ivs, lo, hi, cc)
    type(box3_t), intent(inout) :: boxes(:)
    integer, intent(in)         :: id, nb, ivs(:), lo(3), hi(3)
    real(dp), intent(out)       :: cc(:,:,:,:)
    integer                     :: nc, nb_id, lo_nb(3), hi_nb(3)

    nc    = boxes(id)%n_cell
    lo_nb = lo + a3_nb_dix(:, nb) * nc
    hi_nb = hi + a3_nb_dix(:, nb) * nc
    nb_id = boxes(id)%neighbors(nb)
    cc    = boxes(nb_id)%cc(lo_nb(1):hi_nb(1), lo_nb(2):hi_nb(2), &
         lo_nb(3):hi_nb(3), ivs)
  end subroutine a3_get_cc_from_nb

  ! Fill values on side from a neighbor
  subroutine a3_gc_side_from_nb(boxes, id, nb, ivs)
    type(box3_t), intent(inout) :: boxes(:)
    integer, intent(in)         :: id, nb, ivs(:)
    integer                     :: nc, nb_id
  ...
  end subroutine a3_gc_side_from_nb

  ! Fill ghost cells for variables ivs(:) on the corners of all boxes, using
  ! subr_no_nb on refinement boundaries and subr_bc on physical boundaries
  subroutine a3_gc_corners(tree, ivs, subr_no_nb, subr_bc)
    type(a3_t), intent(inout) :: tree
    integer, intent(in)       :: ivs(:)
    procedure(a3_subr_gc)     :: subr_no_nb, subr_bc
    integer                   :: lvl, i, id
  ...
  end subroutine a3_gc_corners

  ! Fill ghost cells for variables ivs(:) on the corners of a boxes, using
  ! subr_no_nb on refinement boundaries and subr_bc on physical boundaries
  subroutine a3_gc_box_corners(boxes, id, ivs, subr_no_nb, subr_bc)
    type(box3_t), intent(inout) :: boxes(:)
    integer, intent(in)         :: id, ivs(:)
    procedure(a3_subr_gc)       :: subr_no_nb, subr_bc
    integer                     :: cn, nbs(3)
    ...
    do cn = 1, 8
       ...
       if (boxes(id)%neighbors(nbs(1)) > a5_no_box) then
          ...
          call a3_gc_corner_from_nb(boxes, id, cn, nbs(2), ivs)
       else if (boxes(id)%neighbors(nbs(3)) > a5_no_box) then
          call a3_gc_corner_from_nb(boxes, id, cn, nbs(3), ivs)
  ...
  end subroutine a3_gc_box_corners

  ! Bilinear interpolation from a parent to fill a corner ghost cell
  subroutine a3_corners_prolong1(boxes, id, cn, ivs)
    type(box3_t), intent(inout) :: boxes(:)
  ...
  subroutine a3_corners_extrap(boxes, id, cn, ivs)
    type(box3_t), intent(inout) :: boxes(:)
    integer, intent(in)         :: id, cn, ivs(:)
    integer                     :: nc
  ...
  subroutine a3_gc_corner_from_nb(boxes, id, cn, nb, ivs)
    type(box3_t), intent(inout) :: boxes(:)
    integer, intent(in)         :: id, cn, nb, ivs(:)
    integer                     :: nc, nb_id, ijk(3), ijk_nb(3), d

    nb_id     = boxes(id)%neighbors(nb)
    nc        = boxes(id)%n_cell
    ijk       = a3_ch_dix(:, cn) * (nc+1)
    ijk_nb    = ijk
    d         = a3_nb_dim(nb)
    ijk_nb(d) = mod(ijk(d) + nc, 2*nc) ! 0 -> nc and nc+1 -> 1

    boxes(id)%cc(ijk(1), ijk(2), ijk(3), ivs) = &
         boxes(nb_id)%cc(ijk_nb(1), ijk_nb(2), ijk_nb(3), ivs)
  end subroutine a3_gc_corner_from_nb

  ! Restrict fluxes from children to parents on refinement boundaries
  subroutine a3_consistent_fluxes(tree, f_ixs)
    type(a3_t), intent(inout) :: tree
    ...
    do lvl = tree%n_lvls-1, 1, -1
       ...
       do i = 1, size(tree%lvls(lvl)%parents)
          ...
          do nb = 1, 6
             ...
             if (.not. a3_has_children(tree%boxes(nb_id))) &
  ...
  subroutine a3_flux_from_children(boxes, id, nb, f_ixs)
    type(box3_t), intent(inout) :: boxes(:)
    integer, intent(in)         :: id, nb, f_ixs(:)
    integer                     :: nc, nch, c_id, i_ch, i, ic, d, ioff(3)
    ...
    case (1)
       do ic = 1, 4
          ...
          boxes(id)%fx(i, ioff(2)+1:ioff(2)+nch, &
               ioff(3)+1:ioff(3)+nch, f_ixs) = 0.5_dp * ( &
    ...
    case (2)
       do ic = 1, 4
          ...
          c_id = boxes(id)%children(i_ch)
          ioff = nch*a3_ch_dix(:, i_ch)
          boxes(id)%fx(ioff(2)+1:ioff(2)+nch, i, &
               ioff(3)+1:ioff(3)+nch, f_ixs) = 0.5_dp * ( &
    ...
    case (3)
       do ic = 1, 4
          ...
          c_id = boxes(id)%children(i_ch)
          ioff = nch*a3_ch_dix(:, i_ch)
          boxes(id)%fx(ioff(2)+1:ioff(2)+nch, &
               ioff(3)+1:ioff(3)+nch, i, f_ixs) = 0.5_dp * ( &
  ...
  end subroutine a3_flux_from_children

  ! Write the cell centered data of a tree to a vtk file
  subroutine a3_write_tree(tree, filename, cc_names, n_cycle, time)
    use m_vtk
    type(a3_t), intent(in) :: tree
    character(len=*)       :: filename, cc_names(:)
    integer, intent(in)    :: n_cycle
    real(dp), intent(in)   :: time
    integer                :: lvl, bc, bn, bn2, n, n_cells, n_nodes, n_grids
    integer                :: ig, i, j, k, id, n_ix, c_ix
    ...
    allocate(connects(8 * cells_per_box * n_grids))
    ...
    call vtk_geo_xml(vtkf, coords, n_nodes, n_cells, 3, n_cycle, time)
  ...
  end subroutine a3_write_tree

  ! subroutine a3_write_tree(tree, filename, cc_names, cc_units, n_cycle, time)
  !   use m_write_silo
  !   type(a3_t), intent(in)          :: tree
