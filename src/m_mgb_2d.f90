...
!> Multigrid code for 2-dimensional problems
...
module m_mgb_2d
  ...
  type, public :: mg2_t
     ...
     procedure(mg2_box_op), pointer, nopass   :: box_op
     !> Subroutine that performs Gauss-Seidel relaxation on a box
     procedure(mg2_box_gsrb), pointer, nopass :: box_gsrb
  end type mg2_t

  abstract interface
     !> Subroutine that performs A * cc(..., i_in) = cc(..., i_out)
     subroutine mg2_box_op(box, i_in, i_out, i_lsf)
       import
       type(box2_t), intent(inout) :: box !< The box to operate on
     ...
     end subroutine mg2_box_op

     !> Subroutine that performs Gauss-Seidel relaxation
     subroutine mg2_box_gsrb(box, i_phi, i_rhs, i_lsf, redblack_cntr)
       import
       type(box2_t), intent(inout) :: box !< The box to operate on
  ...
  public :: mg2_set
  public :: mg2_fas_fmg
  public :: mg2_fas_vcycle
  public :: mg2_lpl_box
  public :: mg2_gsrb_lpl_box

contains

  !> Store multigrid options in a mg type
  subroutine mg2_set(mg, i_phi, i_phi_old, i_rhs, i_res, i_lsf, &
       n_cycle_down, n_cycle_up, n_cycle_base, &
       sides_bc, my_operator, my_gsrb)
    type(mg2_t), intent(out) :: mg           !< Store multigrid options in this variable
    ...
    !> Routine to call for filling ghost cells near physical boundaries
    procedure(a2_subr_gc)    :: sides_bc
    !> Subroutine that performs the (non)linear operator
    procedure(mg2_box_op)   :: my_operator
    !> Subroutine that performs Gauss-Seidel relaxation on a box
    procedure(mg2_box_gsrb) :: my_gsrb
  ...
  subroutine mg2_fas_fmg(tree, mg)
    type(a2_t), intent(inout)       :: tree !< Tree to do multigrid on
    ...
    do lvl = min_lvl, tree%max_lvl
       ...
       ! Perform V-cycle
       call mg2_fas_vcycle(tree, mg, lvl)
    end do

  end subroutine mg2_fas_fmg
  ...
  !> cells on output
  subroutine mg2_fas_vcycle(tree, mg, max_lvl)
    type(a2_t), intent(inout) :: tree !< Tree to do multigrid on
    type(mg2_t), intent(in)   :: mg   !< Multigrid options
  ...
  subroutine correct_children(boxes, ids, mg)
    ...
    integer                     :: i, id, nc, i_c, c_id, ix_offset(2)
  ...
  subroutine mg2_gsrb_lpl_box(box, i_phi, i_rhs, i_lsf, redblack_cntr)
  ...
  end subroutine mg2_gsrb_lpl_box

  !> Perform Laplacian operator on a box
  subroutine mg2_lpl_box(box, i_in, i_out, i_lsf)
    type(box2_t), intent(inout) :: box !< Box to operate on
