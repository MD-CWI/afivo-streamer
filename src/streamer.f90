...
contains
  ...
  !> Set densities to zero inside electrodes. Finite densities can appear due
  !> to mesh refinement.
  subroutine zero_density_inside_electrode(tree)
    type(af_t), intent(inout) :: tree

    call af_loop_box(tree, zero_density_inside_electrode_box)
  end subroutine zero_density_inside_electrode
