...
contains
  ...
  subroutine photoi_set_src(tree, dt)
    ...
    ! ionization rate.
    call a3_loop_box_arg(tree, set_photoionization_rate, &
    ...
    case ("montecarlo")
       if (phmc_physical_photons) then
          ...
          call phmc_set_src_3d(tree, ST_rng, i_electron_old, i_photo, dt)

       else

          call phmc_set_src_3d(tree, ST_rng, i_electron_old, i_photo)
  ...
  subroutine set_photoionization_rate(box, coeff)
    ...
    do k = 1, nc; do j = 1, nc; do i = 1, nc
       ...
       if (tmp < 0) tmp = 0
       box%cc(i, j, k, i_electron_old) = tmp
    end do; end do; end do
  end subroutine set_photoionization_rate

end module m_photoi_3d
