...
contains
  ...
  subroutine field_initialize(cfg, mg)
    ...
    type(mg3_t), intent(inout) :: mg  !< Multigrid option struct
  ...
  subroutine field_compute(tree, mg, have_guess)
    ...
    type(mg3_t), intent(in)   :: mg ! Multigrid option struct
    ...
    if (.not. have_guess) then
       ...
       call mg3_fas_fmg(tree, mg, .false., have_guess)
    else
       ! Perform cheaper V-cycles
       do i = 1, ST_multigrid_num_vcycles
          call mg3_fas_vcycle(tree, mg, .false.)
       end do
    end if

    ! Compute field from potential
    call a3_loop_box(tree, field_from_potential)

    ! Set the field norm also in ghost cells
    call a3_gc_tree(tree, i_eps, i_electric_fld, a3_gc_interp, a3_bc_neumann_zero)
  ...
  subroutine field_from_potential(box)
    ...
    do k = 1, nc+1; do j = 1, nc+1; do i = 1, nc+1
       ...
       box%fc(i, j, k, 1, electric_fld) = -gradient_3d(box, 2, [i, j, k], 1, i_phi)
       box%fc(i, j, k, 2, electric_fld) = -gradient_3d(box, 2, [i, j, k], 3, i_phi)
       box%fc(i, j, k, 3, electric_fld) = -gradient_3d(box, 2, [i, j, k], 5, i_phi)

    end do; end do; end do

    do k = 1, nc; do j = 1, nc; do i = 1, nc

       Ex = -0.5_dp*(gradient_3d(box, 2, [i, j, k], 1, i_phi) + gradient_3d(box, 2, [i, j, k], 2, i_phi))
       Ey = -0.5_dp*(gradient_3d(box, 2, [i, j, k], 3, i_phi) + gradient_3d(box, 2, [i, j, k], 4, i_phi))
       Ez = -0.5_dp*(gradient_3d(box, 2, [i, j, k], 5, i_phi) + gradient_3d(box, 2, [i, j, k], 6, i_phi))
       box%cc(i, j, k, i_electric_fld) = sqrt(Ex**2+Ey**2+Ez**2)

    end do; end do; end do

  end subroutine field_from_potential

end module m_field_3d
