...
contains
  ...
  subroutine photoi_set_src(tree, dt)
    ...
    ! ionization rate.
    call a2_loop_box_arg(tree, set_photoionization_rate, &
    ...
    case ("montecarlo")
       if (phmc_physical_photons) then
          ...
          call phmc_set_src_2d(tree, ST_rng, i_electron_old, &
  ...
  subroutine set_photoionization_rate(box, coeff)
    ...
    do j = 1, nc; do i = 1, nc
       ...
       if (tmp < 0) tmp = 0
       box%cc(i, j, i_electron_old) = tmp
    end do; end do
  end subroutine set_photoionization_rate

end module m_photoi_2d
