...
def get_args():
    ...
    p.add_argument('-Emin', type=float, default=2.5e6,
...
def find_closest_point(xa, va, xb, vb):
    x = xb - xa
    v = vb - va
    t = -np.dot(x, v) / (v**2).sum()         #np.dot matrix multiply
    x1 = xa + t * va
    x2 = xb + t * vb
...
for i in range(start_index, args.n):
    ...
    # Load data and convert single rows to 2D arrays                           (d contains all points in one .txt file)
    ...
    times[t_i0[i]:t_i0[i]+t_num[i]] = i                                             #times stores variables can easily turns into real time
    n_points += t_num[i]                                                            #n_points is how many points in total
