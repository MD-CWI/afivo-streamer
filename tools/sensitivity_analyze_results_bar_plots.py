...
parser.add_argument('-bar_threshold', type=float, default = 0.05,
               help='Default threshold for the mean of all the variables')
...
if args.bar_plot:
    for i, iy in enumerate(args.y):
        ...
        stats_filtered = stats[np.where(abs(stats[:,i,0]) > args.bar_threshold)]
        filtered_reactions = [used_reactions_list[x] for x in np.where(abs(stats[:,i,0]) > 0.05)[0]]
        ax.barh(np.arange(stats_filtered.shape[0]), stats_filtered[:, i, 0],
                xerr=stats_filtered[:,i,2], align='center', 
                tick_label=filtered_reactions)
        plt.tight_layout()
    plt.show()
