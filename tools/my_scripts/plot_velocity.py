...
p.add_argument("-savgol_width", type=int, default=11, help="Width of savgol filter")
...
for i, log in enumerate(logs):
    ...
    log.plot('y', 'v', ax=axes, label=f'v_log-{i}')
    log.plot('y', 'v_gradient', ax=axes, label=f'v_gradient-{i}')
    log.plot('y', 'v_savgol', ax=axes, label=f'v_savgol-{i}')

plt.xlabel('y')
plt.ylabel('Velocity')
plt.set_xlim(log['y'][0], log['y'][-1])
