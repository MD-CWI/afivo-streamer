...
for i, log in enumerate(logs):
    ...
    log.plot('y', 'velocity', ax=axes[0,0], label=f'velocity-{i}')
    log.plot('y', 'max(E)', ax=axes[1,0], label=f'max(E)-{i}')
    log.plot('y', 'x.2', ax=axes[0,1], label=f'radius-{i}')
    log.plot('y', 'voltage', ax=axes[1,1], label=f'voltage-{i}')
