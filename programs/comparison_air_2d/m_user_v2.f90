...
module m_user
  ...
  type(LT_t), public, protected :: potential_table_right
  integer                       :: upper_electrode = 1
  integer                       :: lower_electrode = 2
  integer                       :: right_boundary = 1
...
contains
  ...
  subroutine potential_from_table()
    ...
    potential_table_right = LT_create(0.0_dp, 0.1_dp, 1000, 1)
    td_file = "applied_voltage_upper_bigdomain.txt";
    ...
    td_file = "applied_voltage_lower_bigdomain.txt";
    call table_from_file(td_file, "location[m]_vs_potential[V]", x_data, y_data)
    call LT_set_col(potential_table, lower_electrode, x_data, y_data)
    td_file = "applied_voltage_right_bigdomain.txt";
    call table_from_file(td_file, "location[m]_vs_potential[V]", x_data, y_data)
    call LT_set_col(potential_table_right, right_boundary, x_data, y_data)
  ...
  subroutine potential_bc(box, nb, iv, coords, bc_val, bc_type)
    ...
    if (af_neighb_dim(nb) == NDIM) then
       ...
       if (nb == af_neighb_highx) then
          bc_type = af_bc_dirichlet
          do n = 1, box%n_cell**(NDIM-1)
             bc_val(n) = field_voltage * &
                  LT_get_col(potential_table_right, right_boundary, coords(2, n));
          end do
          !bc_val = 0.0_dp
       else
