...
contains
  ...
  subroutine my_init_cond(box)
    ...
    integer                    :: i, j, k, n
    real(dp)                   :: rr(3), gas_dens, n_e, density

    do k = 0, box%n_cell+1
       do j = 0, box%n_cell+1
          do i = 0, box%n_cell+1
             rr = af_r_cc(box, [i, j, k])
             gas_dens = 2.5e25 * exp(-rr(3) / scale_height)
             n_e = n_e0 * exp((rr(3) - 60e3) / e_decay_height)

             box%cc(i, j, k, i_electron) = n_e
             box%cc(i, j, k, i_1pos_ion) = n_e

             if (.not. gas_constant_density) then
                do n = 1, size(gas_fractions)
                   box%cc(i, j, k, i_gas_comp(n)) = gas_dens * gas_fractions(n)
                end do
                box%cc(i, j, k, i_gas_dens) = gas_dens
             end if
