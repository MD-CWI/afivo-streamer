...
#        time = int(fname.split('_')[2].split('.')[0])*args.timeStep[0]
        time = int(fname.split('_')[3].split('.')[0])*args.timeStep[0]
        print(fname)
        t = np.loadtxt(args.dbloc[0]+fname, delimiter=' ')
        y = np.interp(args.point[0], t[:,0], t[:,1])
        #print(str(int(args.point[0]*1e3)))
        #np.savetxt('e_vs_t'+str(int(args.point[0]*1e3)), [t.T,y.T])
        eData.append(y)
        tData.append(time)
        #ax.plot(t[:,0], t[:,1], c=linecolor[line_iter])
        ax.plot(time, y, 'bo')
        plt.xlabel('time (sec)')
        plt.ylabel('e(t) (J/m$^3$)')
#print(np.array(eData).size)
np.savetxt('e_vs_t'+str(int(args.point[0]*1e3)), np.array([np.array(tData), np.array(eData)]).T)
