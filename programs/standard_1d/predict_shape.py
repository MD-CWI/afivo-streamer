...
new_n_x = 3*all_data.shape[2]//4 - N
...
for x in data_new[:,2,:]:
    ax[0].plot(x)
    ax[0].set_xlim(0,data_new.shape[2]//N*N)
    ax[0].set_title('shifted data')
...
for i in range(data_new.shape[2]//N):
    data_small[:,:,i] = np.average(data_new[:,:,N*i:N*i+N],axis=2)

for x in data_small[:,2,:]:
    ax[1].plot(x)
    ax[1].set_xlim(0,data_new.shape[2]//N)
    ax[1].set_title('low resolution data')
